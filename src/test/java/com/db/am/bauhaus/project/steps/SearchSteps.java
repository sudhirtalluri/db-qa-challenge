...
public class SearchSteps {
    ...
    public void iTheResultForTheDropDownMenuDisplayed() throws Throwable {
        user.verify_search_drop_down_menu_result_bread_crumb();
    }


    @When("^he searches for a product from category icon$")
    public void heSearchesForAProductFromCategoryIcon() throws Throwable {
        user.search_from_category_icon();
    }

    @Then("^I the result for the category displayed$")
    public void iTheResultForTheCategoryDisplayed() throws Throwable {
        SerenityFacade.getData(SessionVar.SEARCH_RESULT_BREAD_CRUMB);
        user.verify_search_drop_down_menu_result_bread_crumb();
    }

    @When("^he enter \"([^\"]*)\" in search box$")
    public void heEnterInSearchBox(String searchText) throws Throwable {
        user.set_Search_Text_In_Input_Box(searchText);
    }

    @Then("^he sees search suggestions contains \"([^\"]*)\"$")
    public void iSeeElasticSearchResultsContain(String expectedSearchText) throws Throwable {
        user.get_Search_Suggestions_From_Search_Input_Box(expectedSearchText);
    }
